<x-app-layout>
    ...
    <div class="hr-layout">
        ...
        <main class="hr-main">
            <div class="row mb-3">
                <div class="col-12">
                    <h5 class="mb-1">{{ __('Edit User') }}</h5>
                    <p class="mb-0 small text-muted">{{ __('Update user details for') }} <strong>{{ $user->name }}</strong></p>
                </div>
            </div>

            <div class="hr-panel hr-form-container">
                <form action="{{ route('security.users.update', $user) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="d-flex gap-2 mt-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check-circle me-1"></i> {{ __('Update User') }}
                        </button>
